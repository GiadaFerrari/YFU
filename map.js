...
$('.usa').hover(()=>{
   // $('.usaMap').removeClass('hide');
     $('.usaMap').css('opacity', '1');
        console.log('I m gonna punch jp');

    },()=>{
  // $('.usaMap').addClass('hide');
    $('.usaMap').css('opacity', '0');
    console.log('I love jp');})
        ...
        function create(elem) {
            if ($(clicked).hasClass('usa')) {
                ...
                elem[0].posts.forEach((p) => {
                    ...
                    flagN++
                    console.log(flagN)
                    postClone.querySelector('.flag').classList.add('flag'+flagN)
                    p.forEach((post) => {
